...
export default function ProfileInterestsForm({
  ...
  const isCooldownActive = !canUpdateInterests;
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-[15px] font-semibold text-white">Interests</h2>
          <p className="mt-1 text-[13px] text-zinc-500">
            Choose exactly 3 categories to control your feed.
          </p>
          <p className="mt-2 text-[12px] leading-5 text-zinc-400">
            Your own posts always stay in your feed. After saving, interests can
            be changed again in 7 days.
          </p>
          {isCooldownActive && nextUpdateAt && (
            <p className="mt-2 text-[12px] leading-5 text-amber-200">
              You can change them again on{" "}
              {nextUpdateAt.toLocaleString([], {
                dateStyle: "medium",
                timeStyle: "short",
              })}
              .
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {interestOptions.map((option) => {
          ...
          const isDisabled =
            isCooldownActive || (!isSelected && selectedInterests.length >= 3);
      ...
      <div className="flex justify-end">
        <button
          disabled={isPending || isCooldownActive || !hasExactlyThreeInterests}
          onClick={onSave}
          className={[
