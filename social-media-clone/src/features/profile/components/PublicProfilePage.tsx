...
export default function PublicProfilePage() {
  ...
  const { setIsSuccess, setMessage, isSuccess, message, closeMessage } =
    useStatusMessage();
  ...
  } = api.connection.connectionWithUser.useQuery(
  ...
  if (error || connectionError || !user) return <ServerError />;

  if ("redirecting" in user) return redirect("/profile");

  const isPrivateProfileLocked = !user.isPublic && !connection.isConnected;
