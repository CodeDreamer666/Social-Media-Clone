    ...
    return (
        <div className="fixed top-20 inset-x-0 z-50 pointer-events-none">
            <div className="mx-auto flex max-w-3xl justify-end px-4 sm:px-6">
                <section
                    className={`pointer-events-auto flex items-center gap-4 rounded-2xl border px-5 py-4 bg-zinc-900/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.35)] transition-all duration-300
                        ${isSuccess
                            ? "border-emerald-500/20"
                            : "border-[#ff453a]/20"
                        }`}
                >
                    <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isSuccess ? "bg-emerald-500/10" : "bg-[#ff453a]/10"}`}>
                        {isSuccess ? (
                    ...
                    <button
                        ...
                        className="flex size-7 shrink-0 cursor-pointer items-center justify-center rounded-full text-zinc-500 transition-colors duration-200 hover:bg-white/5 hover:text-white"
                    >
